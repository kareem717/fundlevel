...
CREATE TABLE
    daily_aggregated_venture_analytics (
        venture_id INT NOT NULL REFERENCES ventures,
        day_of_year INT NOT NULL,
        impressions_count INT NOT NULL DEFAULT 0,
        uniques_impressions_count INT NOT NULL DEFAULT 0,
        favourited_count INT NOT NULL DEFAULT 0,
        created_at timestamptz DEFAULT CLOCK_TIMESTAMP() NOT NULL,
        PRIMARY KEY (venture_id, day_of_year)
    );

SELECT
    cron.schedule (
        'aggregate-daily-venture-analytics',
        '0 4 * * *',
        $$
        WITH
            impression_counts AS (
        ...
        SELECT
            ...
            COALESCE(ic.venture_id, fc.venture_id) AS venture_id,
            COALESCE(ic.impressions_count, 0) AS impressions_count,
            COALESCE(ic.uniques_impressions_count, 0) AS uniques_impressions_count,
            COALESCE(fc.favourited_count, 0) AS favourited_count
        FROM
            impression_counts ic
            FULL OUTER JOIN favourited_counts fc ON ic.venture_id = fc.venture_id;
        $$
    );
...
-- +goose StatementBegin
SELECT
    cron.unschedule ('aggregate-daily-venture-analytics');

DROP TABLE daily_aggregated_venture_analytics;
