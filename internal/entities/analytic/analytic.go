...
type SimplifiedDailyAggregatedVentureAnalytics struct {
	bun.BaseModel `bun:"daily_aggregated_venture_analytics"`
	DayOfYear               int `json:"dayOfYear" min:"1" max:"366"`
	ImpressionsCount        int `json:"impressionsCount" min:"0"`
	UniquesImpressionsCount int `json:"uniquesImpressionsCount" min:"0"`
	FavouritedCount         int `json:"favouritedCount" min:"0"`
...
type DailyAggregatedVentureAnalytics struct {
	bun.BaseModel `bun:"daily_aggregated_venture_analytics"`
	VentureID     int `json:"ventureId" min:"1"`
	SimplifiedDailyAggregatedVentureAnalytics
	CreatedAt time.Time `json:"createdAt"`
}
