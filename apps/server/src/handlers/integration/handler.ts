...
import { createNangoClient } from "@/lib/utils/nango";
			...
			return await Sentry.startSpan(
				...
				async () => {
					...
					try {
						...
						if (span) {
							span.setAttribute(
								"integration.processing_time_ms",
								performance.now() - startTime,
							);
							span.setAttribute("integration.type", integration);
						}
										...
										return await Sentry.startSpan(
											{
												...
												attributes: {
													table: getTableConfig(
														integrationSchema.nangoConnections,
													).name,
												},
											},
											async () => {
												...
												const db = createDB();
												await db
													.insert(integrationSchema.nangoConnections)
													.values({
