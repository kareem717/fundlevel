...
import type { IAccountRepository, ILinkedAccountRepository } from "../../storage";
...
export class LinkedAccountService implements ILinkedAccountService {
  ...
  constructor(mergeApiKey: string, accounRepo: IAccountRepository, linkedAccountRepo: ILinkedAccountRepository) {
  ...
  async swapPublicToken({
    ...
    const token = await this.merge.accounting.accountToken.retrieve(publicToken);
