  ...
  return (
    <ul className="flex-grow overflow-auto py-4">
      {items?.map((item, i) => {
        ...
        const isVariant = Boolean(item.product.enableVariants)
        const variant = item.product.variants.variantProducts.length
          ? item.product.variants.variantProducts.find((v) => v.id === item.variant)
          : undefined

        // const info = isVariant ? (variant as InfoType) : (product. as InfoType)
            ...
            <div className="relative flex w-full flex-row justify-between px-1 py-4">
              <Link className="z-30 flex flex-row space-x-4" href={url}>
                ...
                <div className="flex flex-1 flex-col text-base">
                  ...
                  {isVariant && variant.options?.length ? (
