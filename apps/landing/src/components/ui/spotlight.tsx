...
export function SpotLightItem({ children, className }: SpotlightItemProps) {
  ...
  return (
    ...
    >
      {isHovered && (
        <div
          className="absolute z-50 w-full h-full transition duration-300 opacity-0 pointer-events-none rounded-xl group-hover:opacity-100 "
          style={{
            background: `
            radial-gradient(
      ...
      {HoverFocusSpotlight && (
        <div
          className="absolute inset-0 z-10 bg-fixed rounded-lg opacity-0 group-hover:opacity-100"
          style={{
