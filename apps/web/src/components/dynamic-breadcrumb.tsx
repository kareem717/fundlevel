...
export interface DynamicBreadcrumbProps extends ComponentPropsWithoutRef<typeof Breadcrumb> {
    ...
    <Breadcrumb className={className} {...props}>
      <BreadcrumbList>
        {items.slice(0, -1).map((item, index) => (
          <>
            {item.url ? (
              <BreadcrumbLink key={index} className="hidden md:block" href={item.url}>
                {item.title}
              </BreadcrumbLink>
            ) : (
