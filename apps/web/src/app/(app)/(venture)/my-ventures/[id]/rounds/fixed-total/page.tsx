...
import { getVentureFixedTotalRoundsCursor } from "@/actions/ventures";
...
export default function VentureFixedTotalRoundsPage({ params }: { params: { id: string } }) {
  const [data, setData] = useState<FixedTotalRound[]>([])
  ...
  const { executeAsync, isExecuting } = useAction(getVentureFixedTotalRoundsCursor, {
    onSuccess: ({ data }) => {
      const newData = data?.fixedTotalRounds ?? []
