...
import { getVentureRoundInvestmentsCursor } from "@/actions/ventures";
...
export default function VentureRoundInvestmentsPage({ params }: { params: { id: string } }) {
  ...
  const { executeAsync, isExecuting } = useAction(getVentureRoundInvestmentsCursor, {
    onSuccess: ({ data }) => {
    ...
    onError: (error) => {
      toast.error("Error fetching round investments")
      console.error("Error fetching round investments:", error)
    }
  })
