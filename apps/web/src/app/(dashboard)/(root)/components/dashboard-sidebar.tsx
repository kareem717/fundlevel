...
import { SidebarGroup, SidebarGroupLabel, SidebarMenuItem, SidebarMenuButton, SidebarMenuAction, SidebarMenu as SidebarMenuComponent } from "@fundlevel/ui/components/sidebar";
import { DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem } from "@fundlevel/ui/components/dropdown-menu";
              ...
              (
                <div className="flex flex-col gap-2 px-2">
                  {Array.from({ length: 2 }).map(() => (
                    <Skeleton className="w-full h-6 rounded-sm" key={generate()} />
                  ))}
                </div>
              ) : (
                companies.map((item) => (
                  <SidebarMenuItem key={item.id}>
                    <SidebarMenuButton>
                      ...
                      <Avatar className="rounded-sm size-6">
                        <AvatarFallback className="rounded-none font-bold bg-muted-foreground">{item.name.split(" ").slice(0, 2).map((name) => name[0]).join("")}</AvatarFallback>
                      </Avatar>
                      <span>{item.name}</span>
                    </SidebarMenuButton>
                    <DropdownMenu>
