...
import { LineItemsTable } from "./components/line-table";
...
export default async function AccountingInvoicePage({
  params,
}: { params: Promise<{ companyId: string; billId: string }> }) {
  const { companyId, billId: billIdString } = await params;

  const billId = Number.parseInt(billIdString, 10);
  ...
  const req = await client(env.NEXT_PUBLIC_BACKEND_URL, token).bill[
    ":billId" 
  ].$get({
    param: { billId },
  });
            ...
            <TabsContent value="transactions" className="mt-4">
              <Card className="relative">
                ...
                <CardContent className="mb-20">
                  <TransactionsTable entityType="bill" entityId={billId} />
                </CardContent>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-medium">
                    {formatCurrency(
                      (bill.totalAmount || 0) - (bill.remainingBalance || 0),
                      bill.currency || undefined,
                    )}
                  </p>
                </div>
