...
export const createBusinessSchema = object<CreateBusinessParams>().shape({
	business: object<BusinessParams>().shape({
		foundingDate: date().required("Founding date is required").nonNullable("Founding date is required"),
		displayName: string().min(3).max(100).required("Display name is required").nonNullable("Display name is required"),
