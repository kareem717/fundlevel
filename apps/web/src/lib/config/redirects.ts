...
export const redirects = {
  ...
  app: {
    company: (id: number) => ({
      ...
      bills: {
        index: companyPrefix(id, "bills"),
        show: (billId: number) => companyPrefix(id, `bills/${billId}`),
        reconcile: (billId: number) =>
          companyPrefix(id, `bills/${billId}/reconcile`),
      },
      connections: {
    ...
    root: appRoot,
    createCompany: `${appRoot}?create=true`,
  },
};
