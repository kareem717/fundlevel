...
export class BankAccountRepository implements IBankAccountRepository {
  ...
  async getMany(filter: GetManyBankAccountsFilter) {
    ...
    const whereCondition = and(
      filter.companyIds
        ? inArray(bankAccounts.companyId, filter.companyIds)
        : undefined,
    );
    ...
    switch (filter.sortBy) {
      case "transactions":
        qb = qb
          ...
          .orderBy(
            filter.order === "asc"
              ? asc(count(bankTransactions.id))
              : desc(count(bankTransactions.id)),
          );
        break;
      case "id":
        qb = qb.orderBy(
  ...
  async get(filter: { id: number } | { remoteId: string }) {
    ...
    const [data] = await this.db
      ...
      .where(
        "id" in filter
          ? eq(bankAccounts.id, filter.id)
          : eq(bankAccounts.remoteId, filter.remoteId),
      );

    return data;
  }

  async upsertMany(params: CreateBankAccountParams[], companyId: number) {
    if (params.length === 0) {
      return;
    }
    ...
    await this.db
      .insert(bankAccounts)
      .values(params.map((bankAccount) => ({ ...bankAccount, companyId })))
      .onConflictDoUpdate({
        ...
        set: {
          ...
          officialName: sql.raw(`excluded.${bankAccounts.officialName.name}`),
