 ...
 */
export const getBankAccountsByCompanyIdAction = actionClientWithAccount
  .schema(z.number())
...
export const getBankTransactionsByAccountIdAction = actionClientWithAccount
  .schema(z.string())
  .action(async ({ ctx: { api }, parsedInput }) => {
    const req = await api.accounting["bank-accounts"][":id"].transactions.$get({
      param: {
        id: parsedInput,
      },
    });
